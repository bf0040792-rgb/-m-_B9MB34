...
# All 22 APIs integrated correctly
SMS_APIS = [
...
<head>
    ...
    <style>
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #0f0f0f; color: #00ff00; text-align: center; padding: 20px; }
        .container { max-width: 400px; margin: auto; background: #1a1a1a; padding: 20px; border-radius: 10px; border: 1px solid #333; box-shadow: 0 0 20px rgba(0,255,0,0.2); }
        ...
        .stats { margin-top: 20px; text-align: left; background: #222; padding: 10px; border-radius: 5px; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <input type="number" id="count" placeholder="SMS Count (Max 200)">
        <input type="number" id="delay" placeholder="Delay (Seconds)" value="1" step="0.5">
        <br>
    ...
    <script>
        function start() {
            ...
            if(!p || !c) return alert("Fill all fields");
...
def start_bomb():
    ...
    if not status_db["running"]:
        thread = threading.Thread(target=bomb_worker, args=(target, count, delay))
        thread.start()
